...
let isArrayLike = require('lodash/isArrayLike');
 ...
 * @returns {boolean}
 */
export function isObjectPlain(v) {
 ...
 * @returns {boolean}
 */
export function objectOf(val, cls) {
 ...
 * @returns {boolean}
 */
export function isNaturalNum(v) {
    if (typeof v !== 'string') v = String(v)
    return Kreg.patternNaturalNum.test(v)
}

/**
 * 检查haystack是否在needle里面
 * @param needle 数组或对象
 * @param haystack 要查找的值
 * @returns {boolean}
 */
