 ...
 * @returns {string}
 */
export function base64UrlEncode(v) {
    let arr = v
    if (!_.isArray(v)) {
        if (!Kvali.isString(v)) {
            v = Kconv.toString(v)
 ...
 * @returns {string}
 */
export function base64UrlDecode(v, returnArray = false) {
    if (!Kvali.isString(v)) {
        return ''
    }
 ...
 */
export function authcode(data, key, encode = true, expiry = 0) {
    ...
    let keya = key.substring(0, 16)
    let keyb = key.substring(16)
    let keyc = encode ? (md5(Ktime.nowMilli()).substring(32 - Kcons.DYNAMIC_KEY_LEN)) : (data.substring(0, Kcons.DYNAMIC_KEY_LEN))
    ...
    if (encode) {
        ...
        expiry += now
        expMd5 = md5(data + keyb).substring(0, 16)
        data = Kstr.sprintf('%010d', expiry) + expMd5 + data
        data = Kstr.str2Bytes(data)
    } else {
        data = base64UrlDecode(data.substring(Kcons.DYNAMIC_KEY_LEN), true)
    }
    ...
    if (encode) {
        ...
        let expTime = parseInt(res.substring(0, 10))
        let ret = res.substring(26)
 ...
 * @returns {string}
 */
export function easyEncrypt(data, key) {
    if (!Kvali.isString(data) || data === '') {
        return ''
    }
 ...
 * @returns {string}
 */
export function easyDecrypt(data, key) {
