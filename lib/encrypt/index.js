...
function base64UrlDecode(v) {
    ...
    v = _.padEnd(v, v.length + (v.length % 4), '=')
    let r = ''
    try {
        let arr = Base64.toByteArray(v)
        r = Kstr.utf8Arr2Str(arr)
    } catch (e) {
...
function authcode(data, key, encode = true, expiry = 0) {
    ...
    if (encode) {
        ...
        console.log('----------authcode 000:', data)
        console.log('----------authcode 111:', data.substr(Kcons.DYNAMIC_KEY_LEN))
        data = base64UrlDecode(data.substr(Kcons.DYNAMIC_KEY_LEN))
        console.log('----------base64 dec 000:', data)
    }
