 ...
 */
export function imgToBase64(file, resize = 1, encoderOptions = 0.8) {
 ...
 */
export function getDomain(url, firstLevel = false) {
    if (Kvali.isEmpty(url)) {
        url = window.location.href
    } else if (url.indexOf('//') === 0) {
        url = 'http:' + url
 ...
 * @returns {*}
 */
export function getQueryByName(name, url, chkHash = true) {
    if (Kvali.isEmpty(url)) {
        url = window.location.href
    }
